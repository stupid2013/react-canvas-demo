...
class MyCanvas extends React.Component {
  ...
  clearLayer = () => {
    const { layerNode, dispatch, from } = this.props;
    dispatch({
      type: `${from}/stateWillUpdate`,
      payload: {
        selectedShape: 'reset',
  ...
  deleteCurrent = () => {
    const { stageNode, currentShape, dispatch, from } = this.props;
    dispatch({
      type: `${from}/stateWillUpdate`,
      payload: {
  ...
  download = () => {
    const { stageNode, from, dispatch, imgArr } = this.props;
    const imgBase64 = stageNode.toDataURL({
      mimeType: 'image/png',
    });
    const arr = [...imgArr];
    arr.push(imgBase64);
  ...
  render() {
    const { dispatch, image, currentShape, imageHeight, imageNode,
      selectedShape, stageNode, layerNode, from } = this.props;
